	...
	async execute(interaction,client) {
        ...
        if (interaction.options.getSubcommand() === "加密or解密") {
            ...
            try{
                if (DecryptOrEncrypt == "encrypt") {
                    if (EncryptionAlgorithm == "AES") {
                        var EnDeText = CryptoJS.AES.encrypt(text, `${!password ? "" : password}`).toString()
                    }
                    if (EncryptionAlgorithm == "DES") {
                        var EnDeText = CryptoJS.DES.encrypt(text, `${!password ? "" : password}`).toString()
                    }
                    if (EncryptionAlgorithm == "RC4") {
                        var EnDeText = CryptoJS.RC4.encrypt(text, `${!password ? "" : password}`).toString()
                    }
                    if (EncryptionAlgorithm == "Rabbit") {
                        var EnDeText = CryptoJS.Rabbit.encrypt(text, `${!password ? "" : password}`).toString()
                    }
                    if (EncryptionAlgorithm == "TripleDes") {
                        var EnDeText = CryptoJS.TripleDES.encrypt(text, `${!password ? "" : password}`).toString()
                    }
                }
                
                else if (DecryptOrEncrypt == "decrypt") {
                    if (EncryptionAlgorithm == "AES") {
                        var EnDeText = CryptoJS.AES.decrypt(text, `${!password ? "" : password}`).toString(CryptoJS.enc.Utf8)
                    }
                    if (EncryptionAlgorithm == "DES") {
                        var EnDeText = CryptoJS.DES.decrypt(text, `${!password ? "" : password}`).toString(CryptoJS.enc.Utf8)
                    }
                    if (EncryptionAlgorithm == "RC4") {
                        var EnDeText = CryptoJS.RC4.decrypt(text, `${!password ? "" : password}`).toString(CryptoJS.enc.Utf8)
                    }
                    if (EncryptionAlgorithm == "Rabbit") {
                        var EnDeText = CryptoJS.Rabbit.decrypt(text, `${!password ? "" : password}`).toString(CryptoJS.enc.Utf8)
                    }
                    if (EncryptionAlgorithm == "TripleDes") {
                        var EnDeText = CryptoJS.TripleDES.decrypt(text, `${!password ? "" : password}`).toString(CryptoJS.enc.Utf8)
                    }
                }
            } catch {
            ...
            const embed = new MessageEmbed()
                ...
                .addFields({ name: '加密&解密後:', value: '```' + `${!EnDeText ? " " : EnDeText}` + '```'})
                .setFooter({ text: `${client.user.tag} (/text)`, iconURL: client.user.displayAvatarURL() })
                .setTimestamp()
